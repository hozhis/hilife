					...
					success : function(msg) {
						...
						for(var i = 0; i < orders.length; i++){
							...
							if(orders[i].orderType == 0){ //商品订单
								...
								if(orders[i].orderStatus == 101){
									_opts += "需付款￥ " + orders[i].totalAmount/100 + "元</div><button class=\"btn-cancel highlight\" type=\"button\">取消</button>";
								}
								if(orders[i].orderStatus == 201){
									_opts += "实付款￥ " + orders[i].totalAmount/100 + "元</div><button class=\"highlight\" type=\"button\">待发货</button>";
								}
								if(orders[i].orderStatus == 301){
									_opts += "实付款￥ " + orders[i].totalAmount/100 + "元</div><button class=\"highlight\" type=\"button\">确认收货</button>";
								}
								if(orders[i].orderStatus == 501){
									_opts += "需付款￥" + orders[i].totalAmount/100 + "元</div><button class=\"highlight\" type=\"button\">待付款</button>";
								}
								if(orders[i].orderStatus == 601){
									_opts += "实付款￥ " + orders[i].totalAmount/100 + "元</div><button class=\"btn-evaluate highlight\" type=\"button\">待评价</button>";
								}
								_opts += "<button class=\"btn-detail\" type=\"button\">查看详情</button></div></li>";
							}
						}
